...
case class Solver(val board: Board) extends SolverPieceOrderer {
  ...
  /**
    ...
    * @return
    */
  protected def recursiveSolver(
      piecesLeftToPut: List[(Piece, Int)],
      availablePlaces: BitSet = board.bitsetWithAllFields,
      alreadyPlacedPieces: BitSet = BitSet(),
      solutionAccumulator: Map[Piece, BitSet] = Map()
